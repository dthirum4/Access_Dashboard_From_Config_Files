...
import Classfiles.Select_credentials;

public class Push_Elapsed_Time_to_Snowflake {
	public static void add_values_to_snowflake_table(List<String> values) throws IOException, Exception {
		...
		String account = Select_Credentials_from_TestData.get_snowflake_credentials(0, 0);
		String user = Select_Credentials_from_TestData.get_snowflake_credentials(1, 0);
		String privateKeyFile = ".\\Authentication\\oiba_dev_snow_enc.p8";
		String privateKeyPassword = Select_Credentials_from_TestData.get_snowflake_credentials(2, 0);
		String Database = Select_Credentials_from_TestData.get_snowflake_credentials(3, 0);
		String Warehouse = Select_Credentials_from_TestData.get_snowflake_credentials(4, 0);
		String Role = Select_Credentials_from_TestData.get_snowflake_credentials(5, 0);
		String Schema = Select_Credentials_from_TestData.get_snowflake_credentials(6, 0);
		...
		String insertSQL = "INSERT INTO OIBA_DEV_ODW_DB.CURATED.DASHBOARD_PERFORMANCE_MONITOR (DASHBOARD_NAME, REPORT_LOAD_START_TIME,REPORT_LOAD_END_TIME,REPORT_ELAPSED_TIME,REPORT_PERFORMANCE_STATUS) VALUES ('"
				+ values.get(0) + "', '" + values.get(1) + "', '" + values.get(2) + "', " + values.get(3) + ", '"
				+ values.get(4) + "')";
		statement.executeUpdate(insertSQL);
		System.out.println("Data inserted successfully in table for " + values.get(0) + " Report");
